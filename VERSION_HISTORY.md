...
* **Multi-Scene Deletion**: Added a feature to delete multiple scenes at once. If multiple items are selected in the tree view, the plugin gathers all nested scenes recursively, prompts the user once with a localized list of all scenes to delete, and deletes them programmatically.
* **Delete Key Shortcuts**: Connected the `Delete` and `Backspace` keys directly to the deletion handler when focused on the scene tree view.
