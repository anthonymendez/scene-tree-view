...
In general, do not use C-style casts. Instead, use these C++-style casts when explicit type conversion is necessary. Some are part of the standard C++ language, and some are defined in [`absl/base/casts.h`](https://github.com/abseil/abseil-cpp/blob/master/absl/base/casts.h). Use the first that **correctly applies**:
...
if (auto it = my_map\_.find(key); it != my_map\_.end()) {
...
Explicit types for elements of `map`\-like containers are easy to get wrong. In those cases [structured bindings](#Structured_Bindings) for the key and value are almost always clearer than a single variable representing the pair.
...
See [TotW #232](https://abseil.io/tips/232) for more details on when to use `auto` for variable declarations.
...
private:
std::string table_name\_; // OK - underscore at end.
static Pool<TableInfo>\* absl_nullable pool\_; // OK.
};
...
void set_some_var(int var) { some_var\_ = var; }
int some_var() const { return some_var\_; }

private:
bool SomeInternalFunction();

int some_var\_;
int some_other_var\_;
};
...
MyClass::MyClass(int var) : some_var\_(var) {
DoSomething();
}
...
: some_var\_(var), some_other_var\_(var + 1) {
DoSomething();
}
...
// and align them:
MyClass::MyClass(int var)
: some_var\_(var), // 4 space indent
some_other_var\_(var + 1) { // lined up
DoSomething();
}
