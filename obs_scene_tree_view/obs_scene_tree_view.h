...
// Displays scenes, custom folders, allows reordering/nesting, and maps
// selection and context menu actions directly into active OBS frontend
// contexts.
...
protected slots:
  ...
  // Recursively restores the folder expansion states stored in the model.
  void RestoreExpansionStates(QStandardItem *parent = nullptr);

  // Spawns a custom context menu for the scene tree item at the specified
  // position.
...
private:
  ...
  // Last scrollbar positions to preserve state across reloads.
  int last_v_scroll_ = 0;
  int last_h_scroll_ = 0;
  ...
  // Static save/load callback forwarder registered into the OBS frontend
  // system.
  inline static void obs_frontend_save_cb(obs_data_t *save_data, bool saving,
                                          void *private_data) {
    reinterpret_cast<ObsSceneTreeView *>(private_data)
        ->ObsFrontendSave(save_data, saving);
  }
