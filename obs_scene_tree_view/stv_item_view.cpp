...
void StvItemView::SetItemModel(StvItemModel *model) { this->_model = model; }
...
void StvItemView::EditSelectedItem() { this->edit(this->currentIndex()); }

void StvItemView::mouseDoubleClickEvent(QMouseEvent *event) {
  if (obs_frontend_preview_enabled()) {
    // If preview mode enabled, check whether the option to transition output
    ...
    if (transition_enabled) {
      QStandardItem *item =
          this->_model->itemFromIndex(this->indexAt(event->pos()));
