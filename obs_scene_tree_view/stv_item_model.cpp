...
int StvFolderItem::type() const { return StvItemModel::FOLDER; }

StvSceneItem::StvSceneItem(const QString &text, obs_weak_source_t *weak)
    : QStandardItem(text) {
  this->setDropEnabled(false);
  this->setData(QVariant::fromValue(obs_weak_source_ptr({weak})),
                StvItemModel::OBS_SCENE);
...
int StvSceneItem::type() const { return StvItemModel::SCENE; }
...
QMimeData *StvItemModel::mimeData(const QModelIndexList &indexes) const {
  ...
  for (const auto &index : indexes) {
    ...
    item_mime_dat.Data =
        item_mime_dat.Type == QITEM_TYPE::FOLDER
            ? (void *)item
...
bool StvItemModel::dropMimeData(const QMimeData *data, Qt::DropAction action,
  ...
  for (int i = 0; i < num_indexes; ++i) {
    ...
    if (item_data->Type == SCENE)
      this->MoveSceneItem((obs_weak_source_t *)item_data->Data, row,
                          parent_item);
    else
...
void StvItemModel::UpdateTree(obs_frontend_source_list &scene_list,
  ...
  for (size_t i = 0; i < scene_list.sources.num; i++) {
    ...
    if (!scene_it->second) {
      ...
      if (selected) {
        assert(selected->type() == QITEM_TYPE::SCENE ||
               selected->type() == QITEM_TYPE::FOLDER);
...
void StvItemModel::SetSelectedScene(QStandardItem *item, bool set_preview_scene,
  ...
  obs_weak_source_t *weak =
      item->data(QDATA_ROLE::OBS_SCENE).value<obs_weak_source_ptr>().ptr;
...
QStandardItem *StvItemModel::GetCurrentSceneItem() {
  ...
  if (auto scene_it = this->_scenes_in_tree.find(weak);
      scene_it != this->_scenes_in_tree.end())
...
void StvItemModel::SaveSceneTree(obs_data_t *root_folder_data,
  ...
  OBSDataArrayAutoRelease folder_data =
      this->CreateFolderArray(*this->invisibleRootItem(), view);
...
bool StvItemModel::IsManagedScene(obs_source_t *scene_source) const {
  ...
  return obs_data_get_bool(settings, "custom_size") ==
         false /*&&
...
bool StvItemModel::MoveIndexByOne(const QModelIndex &index, int delta) {
  ...
  if (item->type() == SCENE) {
    obs_weak_source_t *weak =
        item->data(OBS_SCENE).value<obs_weak_source_ptr>().ptr;
...
void StvItemModel::MoveSceneItem(obs_weak_source_t *source, int row,
  ...
  if (const auto scene_it = this->_scenes_in_tree.find(source);
      scene_it != this->_scenes_in_tree.end()) {
...
void StvItemModel::MoveSceneFolder(QStandardItem *item, int row,
  ...
  for (int sub_row = 0; sub_row < item->rowCount(); ++sub_row) {
    ...
    if (sub_item->type() == FOLDER)
      ...
      obs_weak_source_t *weak =
          sub_item->data(OBS_SCENE).value<obs_weak_source_ptr>().ptr;
  ...
  for (int i = 0; i < folder.rowCount(); ++i) {
    ...
    if (item->type() == FOLDER) {
      OBSDataArrayAutoRelease sub_folder_data =
          this->CreateFolderArray(*item, view);
      obs_data_set_array(item_data, SCENE_TREE_CONFIG_FOLDER_DATA.data(),
                         sub_folder_data);
      obs_data_set_bool(item_data, SCENE_TREE_CONFIG_FOLDER_EXPANDED.data(),
                        view->isExpanded(item->index()));
      obs_data_set_string(item_data, SCENE_TREE_CONFIG_ITEM_NAME_DATA.data(),
                          item->text().toStdString().c_str());
    } else {
      obs_weak_source_t *weak =
          item->data(QDATA_ROLE::OBS_SCENE).value<obs_weak_source_ptr>().ptr;
...
void StvItemModel::LoadFolderArray(
  ...
  for (size_t i = 0; i < item_count; ++i) {
    ...
    const char *item_name =
        obs_data_get_string(item_data, SCENE_TREE_CONFIG_ITEM_NAME_DATA.data());
    OBSDataArrayAutoRelease folder_data =
        obs_data_get_array(item_data, SCENE_TREE_CONFIG_FOLDER_DATA.data());
    ...
    if (!folder_data) {
      ...
      if (obs_data_get_bool(item_data,
                            SCENE_TREE_CONFIG_FOLDER_EXPANDED.data()))
        expandable_folders.push_back(new_folder_item);
    }
  }
}

void StvItemModel::SetIcon(const QIcon &icon, QITEM_TYPE item_type,
                           QStandardItem *item) {
  if (!item)
    return;
